...
/*			printf ("Debug msg: About to start inner while loop, to read individual entity data\n");*/
			while (fgets (whole_line, 4*MAX_LEN + 1, specific_entities_list)) {
				...
				int ent_initX, ent_initY, floorX, floorY;
				indc = 0;
...
/*
void updateMap (int **matrix, int floorX, int floorY, int entWidth, int entHeight) {
	int indr, indc;

	for (indr = floorY; (indr < floorY+entHeight) && (indr < WIN_HEIGHT); indr ++) {
		for (indc = floorX; (indc < floorX+entWidth) && (indc < WIN_HEIGHT); indc ++) {
			*((*matrix)+(entWidth*indr)+indc) = 1;
			if (((entWidth*indr) + indc) > 196) {
				printf ("Updated the map beyond horizon\n");
			}
		}
	}
}*/
